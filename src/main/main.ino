...
const uint8_t DHTTYPE = DHT22; // Тип датчика DHT 22
BH1750 lightMeter;

const uint8_t ONE_WIRE_BUS = 47;
...
const uint8_t OUT_0 = 34;
const uint8_t OUT_1 = 35;
const uint8_t OUT_2 = 36;
const uint8_t OUT_3 = 37;
const uint8_t OUT_4 = 38;
const uint8_t OUT_5 = 39;
const uint8_t OUT_6 = 40;
const uint8_t OUT_7 = 41;

const uint8_t auto_man = 13;

uint8_t TemperatureSensorHiPins[2] = {42, 43}; // Массив входов от верхних датчиков
uint8_t TemperatureSensorLowPins[2]= {44, 45}; // Массив входов от нижних датчиков
...
uint8_t TemperatureSensorHiValueTemperature[2]; // Массив значений температур от верхних датчиков
uint8_t TemperatureSensorHiValueHumidity[2]; // Массив значений влажности от верхних датчиков
uint8_t TemperatureSensorLowValueTemperature[2]; // Массив значений температур от нижних датчиков
uint8_t TemperatureSensorLowValueHumidity[2]; // Массив значений влажности от верхних датчиков


// Установочные данные
uint8_t setTemperature; // Установленная температура
uint8_t setTemperatureDelta; // Дельта температуры
uint8_t setTemperatureDifference; // Уставка разницы температуры между верхом и низом теплицы
uint8_t setHumidity; // Установленная влажность воздуха
uint8_t setHumidityDifference; // Дельта влажности
uint8_t setLighting; // Уставка освещения
uint32_t mOffset = 5000;

// Лимиты 
uint8_t lowerTemperatureLimit; // Нижний лимит по температуре(заданная температура - дельта температуры)
uint8_t upperTemperatureLimit; // Верхний лимит по температуре(заданная температура + дельта температуры)
uint8_t lowerHumidityLimit; // Нижний лимит по влажности(заданная влажность - дельта влажности)
uint8_t upperHumidityLimit; // Верхний лимит по влажности(заданная влажность + дельта влажности

// Реальные показания
uint8_t realTemperatureHi; // Реальная температура верхних датчиков(усреднённое значение)
uint8_t realTemperatureLow; // Реальная температура нижних датчиков(усреднённое значение)
uint8_t realTemperatureDifference; // Реальная разница температуры верхних и нижних датчиков(Переменная реальной температуры верхних датчиков - Переменная реальной температуры нижних датчиков)
uint8_t realHumidity; //Реальная влажность воздуха
